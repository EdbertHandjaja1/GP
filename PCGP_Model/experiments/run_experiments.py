...
#     seed = np.random.seed(42 + 1000 * rep)  
...
#     Y_train += noise_level * np.std(Y_train, axis=0, keepdims=True) * np.random.randn(*Y_train.shape)
...
def generate_data_for_rep(rep, ns, function, ntest, noise_level):
    """Generate all data for a single repetition across all sample sizes"""
    seed = 42 + 1000 * rep
    np.random.seed(seed)
    ...
    # generate the maximum amount of data needed
    max_n = max(ns)
    ...
    return {
        'seed': seed,
        'meta': meta,
        'theta_train_full': theta_train_full,
...
def run_experiment(n, function, output_idx_to_model, rep):
    ...
    """
    # data for repetition
    data = generate_data_for_rep(rep, ns, function, ntest, noise_level)
    
    X_train = data['X_train_full'][:n]
    Y_train = data['Y_train_full'][:n]
