...
class PrincipalComponentGaussianProcessModel:
    ...
    def _standardize_output(self, Y):
        ...
        Y_centered = Y - self.standardization_mean
        self.standardization_scale = np.sqrt(np.mean(Y_centered**2))
        if self.standardization_scale == 0: self.standardization_scale = 1.0
        return Y_centered / self.standardization_scale
    ...
    def compute_principal_components(self, Y_standardized):
        ...
        s, u, v = tf.linalg.svd(y_tensor, full_matrices=False)
        actual_components = min(self.n_components, s.shape[0], v.shape[0])
        self.n_components = actual_components # Update n_components if data is smaller
        
        phi_basis = tf.transpose(v[:actual_components, :]).numpy()
        weights = (u[:, :actual_components] @ tf.linalg.diag(s[:actual_components])).numpy()
        return weights, phi_basis
    
    def _build_kernel_matrix(self, X1, X2, component_idx):
        variance = 1.0 / self.lambda_w[component_idx]
        rho = self.rho[component_idx, :]
        kernel = GaussianKernel(variance=variance, rho=rho)
        return kernel(X1, X2)

    def _negative_log_likelihood_decomposed(self, params):
        # 1. Unpack and transform parameters
        rho_len = self.n_components * self.input_dim
        lambda_len = self.n_components
        
        self.rho = np.exp(params[:rho_len]).reshape((self.n_components, self.input_dim))
        self.lambda_w = np.exp(params[rho_len : rho_len + lambda_len])
        self.noise_var = np.exp(params[-1])

        n_train = self.X_train_std.shape[0]
        total_nll = 0.0

        # 2. Loop through each component and sum their NLL
        for k in range(self.n_components):
            w_k = tf.constant(self.weights[:, k:k+1], dtype=tf.float64)
            C_k = self._build_kernel_matrix(self.X_train_std, self.X_train_std, component_idx=k)
            Sigma_k = C_k + tf.eye(n_train, dtype=tf.float64) * self.noise_var
            
            try:
                L_k = tf.linalg.cholesky(Sigma_k)
            except tf.errors.InvalidArgumentError: # Matrix is not positive definite
                return np.inf # Return a large number to steer optimizer away
            ...
            total_nll += 0.5 * (data_fit_k + log_det_k + n_train * np.log(2.0 * np.pi))
    ...
    def fit(self, X_train, Y_train, ranges):
        ...
        # ⭐ Smarter initialization based on data statistics
        initial_rho = np.ones((self.n_components, self.input_dim)) * 0.5
        # Use variance of weights to inform signal variance and noise
        weights_var = np.var(self.weights, axis=0)
        initial_lambda_w = 1.0 / np.maximum(weights_var, 1e-6)
        initial_noise_var = np.mean(weights_var) * 0.01 # Start with noise as 1% of signal
        
        initial_params_flat = np.concatenate([
            np.log(initial_rho.flatten()),
            np.log(initial_lambda_w),
            np.array([np.log(initial_noise_var)])
        ])
        
        print("🚀 Starting hyperparameter optimization with informed initial values...")
        result = minimize(
            fun=self._negative_log_likelihood_decomposed,
            x0=initial_params_flat,
            method='L-BFGS-B',
            options={'disp': True, 'maxiter': 2000}
        )
        print(f"\n✅ Optimization finished. Final NLL: {result.fun:.4f}")
        
        # Store final optimized parameters
        rho_len = self.n_components * self.input_dim
        lambda_len = self.n_components
        self.rho = np.exp(result.x[:rho_len]).reshape((self.n_components, self.input_dim))
        self.lambda_w = np.exp(result.x[rho_len : rho_len + lambda_len])
    ...
    def predict(self, X_new, ranges, return_std=False):
        ...
        for k in range(self.n_components):
            ...
            C_k_train = self._build_kernel_matrix(self.X_train_std, self.X_train_std, k)
            Sigma_k_inv = tf.linalg.inv(C_k_train + tf.eye(self.X_train_std.shape[0], dtype=tf.float64) * self.noise_var)
            
            c_k_new_train = self._build_kernel_matrix(X_new_std, self.X_train_std, k)
            
            # Predictive mean for component k
            mu_k = tf.matmul(c_k_new_train, Sigma_k_inv @ w_k)
            mu_g[:, k] = tf.squeeze(mu_k).numpy()

            # Predictive variance for component k (latent function)
            c_k_new_new_diag = tf.linalg.diag_part(self._build_kernel_matrix(X_new_std, X_new_std, k))
            v_k = c_k_new_train @ Sigma_k_inv
            var_k = c_k_new_new_diag - tf.reduce_sum(v_k * c_k_new_train, axis=1)
            var_g[:, k] = tf.maximum(var_k, 1e-9).numpy()
        ...
        # ⭐ FIX: Add observation noise for the final predictive variance
        var_y_pred_std = var_y_std + self.noise_var
        
        std_y = np.sqrt(var_y_pred_std) * self.standardization_scale
        
        return mean_y, std_y.numpy()

class GaussianKernel:
    def __init__(self, variance=1.0, rho=None):
        self.variance = tf.constant(variance, dtype=tf.float64)
        self.rho = tf.constant(rho, dtype=tf.float64)

    def __call__(self, X1, X2):
        scaled_sq_diff = tf.square(X1[:, None, :] - X2[None, :, :]) / tf.square(self.rho)
        exponent = -0.5 * tf.reduce_sum(scaled_sq_diff, axis=-1)
        return self.variance * tf.exp(exponent)

# This is the test function from your script
def generate_test_data(n_train=50, n_test=20, input_dim=3, output_dim=5):
    np.random.seed(42)
    X_train = np.random.uniform(0, 1, (n_train, input_dim))
    ranges = [(0, 1)] * input_dim
    ...
    def true_func(x):
        return np.column_stack((
            ...
            x[:, 0] * x[:, 2],
            np.cos(x[:, 1] + x[:, 2]),
            np.exp(x[:, 0]) # Using exp(x) as in your test script
        ))
